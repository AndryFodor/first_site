           ...
           >
               { ({errors, touched, status}) => (
                   <Form>
                       ...
                       <div>
                        {/* загальні помилки (що стосуються всієї форми) можна зберігати в стейті і передавати через пропси, а можна скористатися тим інструментарієм, який нам пропонує бібліотека  Formik. Просто в функцію, яка передається як коллбек в onSubmit, слід прийняти також об'єкт setStatus, при виклиці якого з повідомленням це передане повідомлення прийде у властивості status*/}
                        {props.errors}
                        {status} 
                       </div>
                   </Form>
