...
// це постійна фейкова зміна стейту, яка необхідна для демонстрації роботи функції MapStateToProps. Змінюючи щосекундно стейт, в консолі ми можемо побачити вивід повідомлення, що спрацювала функція MapStateToProps. Вона завжди та всюди спрацьовує, колив якомусь місці змінюється стейт. Але важливо, що ререндерінг кожної сторінки не відбувається через то, що функція MapStateToProps звіряє попереднє значення з тільки що отриманим, і якщо в результаті перевірки на рівність отримується значення true, то ця функція не стимулює ререндерінг
// setInterval(() => {
